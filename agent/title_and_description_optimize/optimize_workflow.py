...
class OptimizeState(BaseModel):
    ...
    # 错误信息
    create_title_error_message: str = ""
...
async def get_keywords(state: OptimizeState):
    ...
    keywords = GetKeywordsAgent().invoke(product_title, product_description)
    print(keywords)
    # 假如缺少任何一个都向客户进行询问

    # 2. 从别的商店那里来(补充商品属性)
    # related_products_urls = state.related_products_urls


async def create_title(state: OptimizeState):
    # 品牌名称+产品类型+产品属性+详细信息(有热点信息则存放热点信息)    /优点/使用地点,字符限制为255个字符,使用“ ( ”和“ / ”等分隔符有助于你的标题更易于阅读。但是尽量少在分隔符周围留空格。
    # 雨树芒果茶袋红茶，香气清新->Raintree芒果红茶茶包（100杯/袋/茶包），清真认证，锡兰天然水果茶，每天作为冰茶或热茶饮用，味道清爽，香气浓郁。

    # 根据是否有错误信息判断进行创建还是修改
    if state.create_title_error_message == "":
        # 创建
        pass
    else:
        # 修改
        pass
    # capitalizemytitle ->大写词的首个字符

    return {"create_title_error_message": ""}
...
async def invoke_optimize_workflow(product_url: str, product_title: str = "", product_description: str = ""):
    """
    ...
    app = get_app()
    result = await app.ainvoke(OptimizeState(product_url=product_url, product_title=product_title, product_description=product_description), config=configuration, stream_mode="values")
    return result
